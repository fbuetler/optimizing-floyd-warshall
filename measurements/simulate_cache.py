...
def access(i: int, j: int, n:int, data_size: int, t: int, operation: str) -> int:
    # compute tag and set identifiers
    # HACK: Assume cache line size to be 64 bytes and divisible by sizeof(datatype)
    tag = (i * n + j) // (64//data_size)
    set = tag % cache["S"]
